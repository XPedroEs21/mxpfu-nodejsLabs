...
router.get("/:email",(req,res)=>{
  const email = req.params.email;                                        // Extract the email parameter from the request URL
  let filtered_users = users.filter((user) => user.email === email);     // Filter the users array to find users whose email matches the extracted email parameter
  res.send(filtered_users);                                               // Send the filtered_users array as the response to the client
});
